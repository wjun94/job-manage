...
class App extends React.Component<RouteComponentProps, S> {
  ...
  render() {
    ...
    return (
      <>
        <Form {...layout} name="basic" onFinish={this.onFinish} ref={(el) => this.formRef = el} className='company-info app-container animate'>
          <Form.Item
            label="称呼"
            name="name"
          ...
          <Col offset={6}>
            ...
            <Button htmlType="submit" type="primary">{id ? '更新' : '添加'}</Button>
          </Col>
        </Form >
      </>
    );
