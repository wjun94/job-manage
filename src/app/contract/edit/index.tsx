...
class App extends React.Component<RouteComponentProps> {
  ...
  onFinish = async (values: Node) => {
    ...
    await window.$api.createService({
      ...
      effectAt: values.createAt,
    })
    // 创建广告
    const { list } = this.state
    list.length &&
      list.forEach(async (item: AdvtgNode) => {
    ...
    return (
      <>
        <Form
          ...
          className="contract-edit animate"
        >
          ...
          <div className="box left-line">
            ...
            <Form.Item className="type" label="合同类型" name="type">
              <Radio.Group onChange={this.onType} buttonStyle="outline">
                {contractArr.map((v) => (
                  <Radio.Button key={v.label} value={v.value}>
                    {v.label}
                  </Radio.Button>
                ))}
              </Radio.Group>
            ...
            <div className="flex combination">
              ...
              >
                <Select allowClear placeholder="请选择到账时间">
                  {paymentTimeArr.map((item) => (
                    <Option key={'rcoc-' + item.value} value={item.value}>
                      {item.label}
                    </Option>
                  ))}
                </Select>
              ...
              <Form.Item label=" ">
                <Button onClick={this.onCreateAtBtn} type="primary">
                  当前时间
                </Button>
              </Form.Item>
            </div>
          ...
          <div className="box left-line">
            ...
            <div className="flex">
              ...
              <Form.Item
                label="在线职位"
                rules={[{ required: true, message: '请输入在线职位' }]}
                name="post"
              >
