...
import { setCurrent, setList, setPageSize, init, setPagination, setPaginationProps } from '@/store/customer/being/action'
...
@(connect((state: any) => {
    return ({ ...state.customerBeingReducer })
}, (dispatch) => ({
    setList(list: []) {
...
export default class Home extends React.Component<P, any> {
    ...
    getData = async () => {
        ...
        const { data, total } = await window.$api.customerBeingList({ current: current, pageSize: pageSize })
