...
import { setCurrent, setList, setPageSize, init, setPagination, setPaginationProps } from '@/store/customer/expected/action'
...
@(connect((state: any) => {
    return ({ ...state.customerExpectedReducer })
}, (dispatch) => ({
    setList(list: []) {
...
export default class Home extends React.Component<P, any> {
    ...
    getData = async () => {
        ...
        const { data, total } = await window.$api.customerExpectedList({ current: current, pageSize: pageSize })
