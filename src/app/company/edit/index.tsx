...
class App extends React.Component<P> {
  ...
  onFinish = async (values: FormNode) => {
    ...
    const params = { ...data, ...values, prov, city, area, amount: Number(values.amount), createAt: moment(values.createAt).format('YYYY') }
  ...
  render() {
    ...
    if (!data) return '';
    if (data.createAt) {
      data.createAt = moment(data.createAt, "YYYY/MM")
    }
    ...
    return (
      <Form {...layout} name="basic" onFinish={this.onFinish} initialValues={dt} className='company-info app-container animate'>
        ...
        >
          <Select>
            {
              scaleArr.map((v, i) => <Option key={'comp-' + v} value={i + 1}>{v}</Option>)
            }
          </Select>
        </Form.Item>
        <Form.Item
